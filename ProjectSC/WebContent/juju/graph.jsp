...
<%
   //쿼리문 준비
   // 게시글 읽어오기
   int uid = 1;   //일부러 고정시켜준 값
   final String SQL_WRITE_SELECT_BY_UID = "SELECT STORE_UID, TO_CHAR(TO_DATE (order_regdate,'yyyy-mm-dd'),'yy-mm-dd'), SUM(order_totalprice) FROM ORDER_TB GROUP BY TO_CHAR(TO_DATE (order_regdate,'yyyy-mm-dd'),'yy-mm-dd'), STORE_UID ;";
%>
<%
//    변수를 담을 공간 준비
   int store_uid = 0;
...
//       pstmt.setInt(1, uid);
      ...
      if(rs.next()){
    	 store_uid = rs.getInt("store_uid");
    	 order_totalprice = rs.getInt("SUM(order_totalprice)");
         Date d = rs.getDate("order_regdate");
         //Time t =rs.getTime("order_regdate");
         //위에 Date, Time 은  java.sql 걸로 돌아감
         order_regdate="";
         if(d != null){
        	 order_regdate = new SimpleDateFormat("yyyyMMdd").format(d);//+" "+
      ...
      var myChart = new Chart(ctx,
            {
               ...
               data : {
                  ...
                  datasets : [ {
                     ...
                     data : [ <%= order_totalprice%>, <%= order_totalprice%>, <%= order_totalprice%>, <%= order_totalprice%>, <%= order_totalprice%>, <%= order_totalprice%> ],    //데이터 수치   현재 uid는 20으로 고정시켜준 값
