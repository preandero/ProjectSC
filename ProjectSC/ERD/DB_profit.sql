...
SELECT * FROM ORDER_TB ot2 ;
SELECT TO_CHAR(order_regdate, 'YYYY-MM-DD') FROM ORDER_TB ot ;

DELETE 

SELECT 
  SUM(ORDER_TOTALPRICE ) totalprice, 
  to_char(ORDER_REGDATE , 'YYYY-MM-DD') AS daily
FROM 
  ORDER_TB
WHERE 
  to_char(ORDER_REGDATE , 'YYYY-MM-DD') >= '2013-05-01' AND to_char(ORDER_REGDATE , 'YYYY-MM-DD') <= '2020-06-13'
  AND 
  STORE_UID = 1
GROUP BY 
...
INSERT INTO ORDER_TB (ORDER_UID, ORDER_TOTALPRICE , ORDER_REGDATE, STORE_UID ) 
VALUES
(SEQ_order_uid.nextval, 10000, (sysdate + 1), 1);

INSERT INTO ORDER_TB (ORDER_UID, ORDER_TOTALPRICE , ORDER_REGDATE, STORE_UID ) 
VALUES
(SEQ_order_uid.nextval, 15000, (sysdate + 2), 1);

INSERT INTO ORDER_TB (ORDER_UID, ORDER_TOTALPRICE , ORDER_REGDATE, STORE_UID ) 
VALUES
(SEQ_order_uid.nextval, 50000, (sysdate + 4), 1);
